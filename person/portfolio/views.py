...
from .models import Project, Blog, Skill, Connection, Services, Profile
...
def index(request):
    ...
    experiences = Services.objects.all().order_by('-start_date')[:5]
    profile = Profile.objects.all().first()
    form = NameForm()
    context = {'projects': projects, 'skills': skills, 'form': form, 'services': experiences, 'profile': profile}
